...
public class UltimateAutoRankPlugin extends JavaPlugin implements org.bukkit.command.TabExecutor {
    ...
    public void onEnable() {
        ...
        // Dependency checks
        boolean ok = (Bukkit.getPluginManager().getPlugin("LuckPerms") != null);
        if (!ok) {
        ...
        boolean ok2 = (Bukkit.getPluginManager().getPlugin("PlaceholderAPI") != null);
        if (!ok2) {
        ...
        boolean ok3 = (Bukkit.getPluginManager().getPlugin("Playtime") != null);
        if (!ok3) {
            String msg = getConfig().getString("messages.missing_papi", "&cPlaceholderAPI 또는 Playtime 플러그인이 필요합니다.");
            getLogger().severe(color(msg));
            getServer().getPluginManager().disablePlugin(this);
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        PromotionManager.RankRule next = promotionManager.getNextRuleFor(seconds);
        if (next == null) {
        ...
        String remainStr = TimeUtil.formatSeconds(remain);
