...
public class TitleCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("삭제")) {
            ...
            if (args.length < 2) {
                sender.sendMessage(Texts.color("&e사용법: /칭호 삭제 <칭호이름>"));
                return true;
            }
            String t = String.join(" ", java.util.Arrays.copyOfRange(args,1,args.length));
            int affected = plugin.getTitleManager().removeTitleGlobal(t);
            sender.sendMessage(Texts.color("&a삭제 완료: &f" + t + " &7(영향 받은 플레이어 수: " + affected + ")"));
            return true;
        }
        ...
        // default
        sender.sendMessage(Texts.color("&e사용법: /칭호, /칭호 생성 <이름>, /칭호 목록"));
        return true;
    }
}
