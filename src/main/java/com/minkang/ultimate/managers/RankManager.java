...
public class RankManager {
    ...
    public long getPlaySeconds(Player p) {
        long hooked = com.minkang.ultimate.util.PlaytimeHook.getPlaySeconds(p);
    ...
    public void tryPromote(Player p) {
        ...
        fut.thenAccept(user -> {
            ...
            if (seconds >= chain.seconds) {
                ...
                String rawMsg = plugin.getConfig().getString("rankup.broadcast_message",
                        "&6%player%&e 님이 &b%next% &e등급으로 자동 승급되었습니다! 축하합니다!"
                );
                final String finalMsg = rawMsg.replace("%player%", p.getName()).replace("%next%", chain.next);
                Bukkit.getScheduler().runTask(plugin, () -> Bukkit.broadcastMessage(Texts.color(finalMsg)));
            }
        });
    }
