...
@EventHandler(priority = org.bukkit.event.EventPriority.HIGHEST) public void onChat(AsyncPlayerChatEvent e){ PlayerData d=plugin.storage().get(e.getPlayer().getUniqueId()); String id=d.getActiveColorId(); if(id==null) return; ColorDef def=service.get(id); if(def==null) return; e.setMessage(Chat.color(def.code)+e.getMessage()+Chat.color("&r")); }
}