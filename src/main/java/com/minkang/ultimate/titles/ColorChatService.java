...
public class ColorChatService {
    private final java.util.Map<String,String> nameToId = new java.util.HashMap<>(); private final Main plugin; private final java.util.Map<String,ColorDef> colors=new java.util.HashMap<>();
...
public void setActive(java.util.UUID uuid,String colorId){ PlayerData d=plugin.storage().get(uuid); d.setActiveColorId(colorId==null?null:colorId.toLowerCase()); plugin.storage().save(uuid,d); }
    ...
    public ColorDef getByNameOrId(String token){ if(token==null) return null; ColorDef d=get(token); if(d!=null) return d; String id=nameToId.get(token); return id==null?null:get(id);}