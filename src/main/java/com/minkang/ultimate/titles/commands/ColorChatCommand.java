...
public class ColorChatCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("지급")) {
            if (args.length < 3) {
                sender.sendMessage(ColorUtil.colorize("&c사용법: /색깔채팅 지급 <색깔> <플레이어>"));
                return true;
            }
            String color = args[1];
            ...
            sender.sendMessage(ColorUtil.colorize("&a지급 완료: " + target.getName() + " ← " + key));
    ...
    public void openGui(Player p) {
        ...
        for (String k : sec.getKeys(false)) {
            ...
            im.setDisplayName(ColorUtil.colorize(display));
    ...
    public java.util.List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        } else if (args.length == 2 && ("지급".equals(args[0]) || "제거".equals(args[0]))) {
            org.bukkit.configuration.ConfigurationSection sec = plugin.getConfig().getConfigurationSection("colorchat.colors");
            for (String k : sec.getKeys(false)) {
                out.add(k);
                String name = sec.getConfigurationSection(k).getString("name");
