...
public class SpawnChecksCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        synchronized (plugin.getStorage()) {
            ...
            leftAfterConsume = Math.max(0, allow - usedNow);
        }

        cooldowns.put(cdKey, now);

        String sub = (kind.equals("legendary") ? "legendary" : "megaboss");
        String cmdToRun = "checkspawns " + sub;

        String runAs = plugin.getConfig().getString("general.spawn_run_as", "player");
        if ("player".equalsIgnoreCase(runAs)) {
            String tempPerm = plugin.getConfig().getString("general.spawn_temp_permission", "");
            int ticks = plugin.getConfig().getInt("general.spawn_temp_permission_ticks", 2);
            if (plugin.getLpUtils().isAvailable() && tempPerm != null && !tempPerm.isEmpty()) {
                plugin.getLpUtils().runAsPlayerWithTempPerm(p, tempPerm, ticks, new Runnable() {
                    @Override
                    public void run() {
