...
public class TitleCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (sub.equalsIgnoreCase("열기")) {
            if (!sender.hasPermission("utr.user.title.open")) { sender.sendMessage(ColorUtil.colorize("&c권한이 없습니다.")); return true; }
        ...
        if (sub.equalsIgnoreCase("아이템")) {
            if (!sender.hasPermission("utr.admin.title.item")) { sender.sendMessage(ColorUtil.colorize("&c권한이 없습니다.")); return true; }
            if (!(sender instanceof Player)) {
                sender.sendMessage("플레이어만 사용 가능합니다.");
                return true;
            }
        ...
        if (sub.equalsIgnoreCase("생성")) {
            if (!sender.hasPermission("utr.admin.title.create")) { sender.sendMessage(ColorUtil.colorize("&c권한이 없습니다.")); return true; }
            if (args.length < 3) {
        ...
        if (sub.equalsIgnoreCase("삭제")) {
            if (!sender.hasPermission("utr.admin.title.delete")) { sender.sendMessage(ColorUtil.colorize("&c권한이 없습니다.")); return true; }
            if (args.length == 2) {
                int slot;
        ...
        if (sub.equalsIgnoreCase("목록")) {
            if (!sender.hasPermission("utr.admin.title.list")) { sender.sendMessage(ColorUtil.colorize("&c권한이 없습니다.")); return true; }
            java.util.List<String> list = plugin.getConfig().getStringList("global_titles");
