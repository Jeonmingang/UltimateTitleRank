...
public class LPUtils {
    ...
    public void runAsPlayerWithTempPerm(Player player, String permission, long ticks, Runnable action) {
        if (permission == null || permission.isEmpty() || api == null) {
            action.run();
            return;
        }
        CompletableFuture<User> fu = api.getUserManager().loadUser(player.getUniqueId());
        fu.thenAccept(new java.util.function.Consumer<User>() {
            @Override
            public void accept(User user) {
                Node add = Node.builder(permission).value(true).build();
                user.data().add(add);
                api.getUserManager().saveUser(user);
